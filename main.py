...
class SamRequest(BaseModel):
    ...
    output_type: Optional[str] = "overlay"  # 添加输出类型参数
...
async def predict(request: SamRequest):
    ...
    try:
        # 1. 解析Base64图片
        logger.info("收到分割请求")
        logger.info(f"输出类型: {request.output_type}")
        ...
        # 新增：将掩码数据以JSON格式输出到终端
        mask_data = {
            "mask_shape": mask.shape,
            "mask_dtype": str(mask.dtype),
            "unique_values": np.unique(mask).tolist(),
            "foreground_pixels": int(np.sum(mask == 1)),
            "background_pixels": int(np.sum(mask == 0)),
            "total_pixels": int(mask.size),
            "points_used": [{"x": p.x, "y": p.y, "label": p.label} for p in request.points]
        }
        
        # 使用json.dumps美化输出[5](@ref)
        mask_json = json.dumps(mask_data, indent=2, ensure_ascii=False)
        logger.info("掩码JSON数据:\n" + mask_json)

        # 4. 根据输出类型生成不同的掩码图像
        if request.output_type == "binary":
            # 生成二值图像：前景为白色，背景为黑色
            mask_array = mask.astype(np.uint8) * 255
            mask_image = Image.fromarray(mask_array, mode='L')
            logger.info("生成二值掩码图像")
        else:
            # 生成叠加掩码：红色半透明
            mask_rgba = np.zeros((mask.shape[0], mask.shape[1], 4), dtype=np.uint8)
            mask_rgba[mask == 1] = [255, 0, 0, 128]  # 红色，半透明
            mask_image = Image.fromarray(mask_rgba, 'RGBA')
            logger.info("生成叠加掩码图像")
